...
rc = 1
keepalive = 60
connect_packet = ecld_test.gen_connect("retain-qos1-test", keepalive=keepalive)
connack_packet = ecld_test.gen_connack(rc=0)

mid = 6
publish_packet = ecld_test.gen_publish("retain/qos1/test", qos=1, mid=mid, payload="retained message", retain=True)
puback_packet = ecld_test.gen_puback(mid)
mid = 18
subscribe_packet = ecld_test.gen_subscribe(mid, "retain/qos1/test", 0)
suback_packet = ecld_test.gen_suback(mid, 0)
publish0_packet = ecld_test.gen_publish("retain/qos1/test", qos=0, payload="retained message", retain=True)
...
try:
    ...
    if ecld_test.expect_packet(sock, "puback", puback_packet):
        ...
        if ecld_test.expect_packet(sock, "suback", suback_packet):
            if ecld_test.expect_packet(sock, "publish0", publish0_packet):
                rc = 0
    sock.close()
finally:
    broker.terminate()
