...
rc = 1
keepalive = 60
connect_packet = ecld_test.gen_connect("retain-clear-test", keepalive=keepalive)
connack_packet = ecld_test.gen_connack(rc=0)

publish_packet = ecld_test.gen_publish("retain/clear/test", qos=0, payload="retained message", retain=True)
retain_clear_packet = ecld_test.gen_publish("retain/clear/test", qos=0, payload=None, retain=True)
mid_sub = 592
subscribe_packet = ecld_test.gen_subscribe(mid_sub, "retain/clear/test", 0)
suback_packet = ecld_test.gen_suback(mid_sub, 0)

mid_unsub = 593
unsubscribe_packet = ecld_test.gen_unsubscribe(mid_unsub, "retain/clear/test")
unsuback_packet = ecld_test.gen_unsuback(mid_unsub)

cmd = ['../../src/eecloud', '-p', '1888']
broker = ecld_test.start_broker(filename=os.path.basename(__file__), cmd=cmd)

try:
    sock = ecld_test.do_client_connect(connect_packet, connack_packet, timeout=4)
