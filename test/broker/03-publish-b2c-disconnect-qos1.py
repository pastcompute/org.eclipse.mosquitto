...
mid = 3265
keepalive = 60
connect_packet = ecld_test.gen_connect("pub-qos1-disco-test", keepalive=keepalive, clean_session=False)
connack_packet = ecld_test.gen_connack(rc=0)

subscribe_packet = ecld_test.gen_subscribe(mid, "qos1/disconnect/test", 1)
suback_packet = ecld_test.gen_suback(mid, 1)

mid = 1
publish_packet = ecld_test.gen_publish("qos1/disconnect/test", qos=1, mid=mid, payload="disconnect-message")
publish_dup_packet = ecld_test.gen_publish("qos1/disconnect/test", qos=1, mid=mid, payload="disconnect-message", dup=True)
puback_packet = ecld_test.gen_puback(mid)

mid = 3266
publish2_packet = ecld_test.gen_publish("qos1/outgoing", qos=1, mid=mid, payload="outgoing-message")
...
try:
    ...
    if ecld_test.expect_packet(sock, "suback", suback_packet):
        ...
        if ecld_test.expect_packet(sock, "publish", publish_packet):
            ...
            if ecld_test.expect_packet(sock, "connack", connack_packet):
