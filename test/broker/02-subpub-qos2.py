...
mid = 530
keepalive = 60
connect_packet = ecld_test.gen_connect("subpub-qos2-test", keepalive=keepalive)
connack_packet = ecld_test.gen_connack(rc=0)

subscribe_packet = ecld_test.gen_subscribe(mid, "subpub/qos2", 2)
suback_packet = ecld_test.gen_suback(mid, 2)

mid = 301
publish_packet = ecld_test.gen_publish("subpub/qos2", qos=2, mid=mid, payload="message")
pubrec_packet = ecld_test.gen_pubrec(mid)
pubrel_packet = ecld_test.gen_pubrel(mid)
pubcomp_packet = ecld_test.gen_pubcomp(mid)

mid = 1
publish_packet2 = ecld_test.gen_publish("subpub/qos2", qos=2, mid=mid, payload="message")
pubrec_packet2 = ecld_test.gen_pubrec(mid)
pubrel_packet2 = ecld_test.gen_pubrel(mid)
pubcomp_packet2 = ecld_test.gen_pubcomp(mid)
    ...
    if ecld_test.expect_packet(sock, "suback", suback_packet):
        ...
        if ecld_test.expect_packet(sock, "pubrec", pubrec_packet):
            ...
            if ecld_test.expect_packet(sock, "pubcomp", pubcomp_packet):
                if ecld_test.expect_packet(sock, "publish2", publish_packet2):
                    sock.send(pubrec_packet2)

                    if ecld_test.expect_packet(sock, "pubrel2", pubrel_packet2):
