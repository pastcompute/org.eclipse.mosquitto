...
rc = 1
keepalive = 4
connect_packet = ecld_test.gen_connect("01-keepalive-pingreq", keepalive=keepalive)
connack_packet = ecld_test.gen_connack(rc=0)

pingreq_packet = ecld_test.gen_pingreq()
pingresp_packet = ecld_test.gen_pingresp()
...
try:
    ...
    if ecld_test.expect_packet(conn, "connect", connect_packet):
        ...
        if ecld_test.expect_packet(conn, "pingreq", pingreq_packet):
            time.sleep(1.0)
