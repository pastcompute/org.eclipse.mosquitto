...
int main(int argc, char *argv[])
{
	...
	memset(&int_db, 0, sizeof(struct eecloud_db));
	...
	if(config.daemon && config.pid_file){
		pid = _eecloud_fopen(config.pid_file, "wt");
		if(pid){
	...
	if(rc) return rc;
	rc = eecloud_security_init(&int_db, false);
	if(rc) return rc;

#ifdef WITH_SYS_TREE
	if(config.sys_interval > 0){
		/* Set static $SYS messages */
		snprintf(buf, 1024, "eecloud version %s", VERSION);
	...
	for(i=0; i<config.listener_count; i++){
		if(config.listeners[i].protocol == mp_mqtt){
			...
			listensock = _eecloud_realloc(listensock, sizeof(ecld_sock_t)*listensock_count);
			if(!listensock){
				mqtt3_db_close(&int_db);
				if(config.pid_file){
					remove(config.pid_file);
	...
	for(i=0; i<int_db.config->listener_count; i++){
		...
		if(int_db.config->listeners[i].ws_protocol){
			_eecloud_free(int_db.config->listeners[i].ws_protocol);
		}
	}
#endif
	...
	if(int_db.bridges){
		_eecloud_free(int_db.bridges);
	}
#endif
	eecloud__free_disused_contexts(&int_db);
	...
	if(listensock){
		...
		_eecloud_free(listensock);
	}

	eecloud_security_module_cleanup(&int_db);
...
int __stdcall WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	argv = _eecloud_malloc(sizeof(char *)*1);
	argv[0] = "eecloud";
