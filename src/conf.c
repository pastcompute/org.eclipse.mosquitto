...
static int _conf_attempt_resolve(const char *host, const char *text, int log, const char *msg)
{
	...
	if(rc != 0){
		...
		if(rc == EAI_SYSTEM){
			if(errno == ENOENT){
				_eecloud_log_printf(NULL, log, "%s: Unable to resolve %s %s.", msg, text, host);
			}else{
				_eecloud_log_printf(NULL, log, "%s: Error resolving %s: %s.", msg, text, strerror(errno));
			}
		}else{
			_eecloud_log_printf(NULL, log, "%s: Error resolving %s: %s.", msg, text, gai_strerror(rc));
		}
#else
...
static void _config_init_reload(struct mqtt3_config *config)
{
	int i;
	/* Set defaults */
	if(config->acl_file) _eecloud_free(config->acl_file);
	...
	if(config->clientid_prefixes) _eecloud_free(config->clientid_prefixes);
	...
	if(config->password_file) _eecloud_free(config->password_file);
	config->password_file = NULL;
	config->persistence = false;
	if(config->persistence_location) _eecloud_free(config->persistence_location);
	config->persistence_location = NULL;
	if(config->persistence_file) _eecloud_free(config->persistence_file);
	config->persistence_file = NULL;
	config->persistent_client_expiration = 0;
	if(config->psk_file) _eecloud_free(config->psk_file);
	...
	if(config->auth_options){
		for(i=0; i<config->auth_option_count; i++){
			_eecloud_free(config->auth_options[i].key);
			_eecloud_free(config->auth_options[i].value);
		}
		_eecloud_free(config->auth_options);
...
void mqtt3_config_cleanup(struct mqtt3_config *config)
{
	...
	if(config->acl_file) _eecloud_free(config->acl_file);
	if(config->auto_id_prefix) _eecloud_free(config->auto_id_prefix);
	if(config->clientid_prefixes) _eecloud_free(config->clientid_prefixes);
	if(config->config_file) _eecloud_free(config->config_file);
	if(config->password_file) _eecloud_free(config->password_file);
	if(config->persistence_location) _eecloud_free(config->persistence_location);
	if(config->persistence_file) _eecloud_free(config->persistence_file);
	if(config->persistence_filepath) _eecloud_free(config->persistence_filepath);
	if(config->psk_file) _eecloud_free(config->psk_file);
	if(config->listeners){
		for(i=0; i<config->listener_count; i++){
			if(config->listeners[i].host) _eecloud_free(config->listeners[i].host);
			if(config->listeners[i].mount_point) _eecloud_free(config->listeners[i].mount_point);
			if(config->listeners[i].socks) _eecloud_free(config->listeners[i].socks);
#ifdef WITH_TLS
			if(config->listeners[i].cafile) _eecloud_free(config->listeners[i].cafile);
			if(config->listeners[i].capath) _eecloud_free(config->listeners[i].capath);
			if(config->listeners[i].certfile) _eecloud_free(config->listeners[i].certfile);
			if(config->listeners[i].keyfile) _eecloud_free(config->listeners[i].keyfile);
			if(config->listeners[i].ciphers) _eecloud_free(config->listeners[i].ciphers);
			if(config->listeners[i].psk_hint) _eecloud_free(config->listeners[i].psk_hint);
			if(config->listeners[i].crlfile) _eecloud_free(config->listeners[i].crlfile);
			if(config->listeners[i].tls_version) _eecloud_free(config->listeners[i].tls_version);
			if(config->listeners[i].ssl_ctx) SSL_CTX_free(config->listeners[i].ssl_ctx);
#endif
#ifdef WITH_WEBSOCKETS
			if(config->listeners[i].http_dir) _eecloud_free(config->listeners[i].http_dir);
#endif
		}
		_eecloud_free(config->listeners);
	}
#ifdef WITH_BRIDGE
	if(config->bridges){
		for(i=0; i<config->bridge_count; i++){
			if(config->bridges[i].name) _eecloud_free(config->bridges[i].name);
			if(config->bridges[i].addresses){
				for(j=0; j<config->bridges[i].address_count; j++){
					_eecloud_free(config->bridges[i].addresses[j].address);
				}
				_eecloud_free(config->bridges[i].addresses);
			}
			if(config->bridges[i].remote_clientid) _eecloud_free(config->bridges[i].remote_clientid);
			if(config->bridges[i].remote_username) _eecloud_free(config->bridges[i].remote_username);
			if(config->bridges[i].remote_password) _eecloud_free(config->bridges[i].remote_password);
			if(config->bridges[i].local_clientid) _eecloud_free(config->bridges[i].local_clientid);
			if(config->bridges[i].local_username) _eecloud_free(config->bridges[i].local_username);
			if(config->bridges[i].local_password) _eecloud_free(config->bridges[i].local_password);
			if(config->bridges[i].topics){
				for(j=0; j<config->bridges[i].topic_count; j++){
					if(config->bridges[i].topics[j].topic) _eecloud_free(config->bridges[i].topics[j].topic);
					if(config->bridges[i].topics[j].local_prefix) _eecloud_free(config->bridges[i].topics[j].local_prefix);
					if(config->bridges[i].topics[j].remote_prefix) _eecloud_free(config->bridges[i].topics[j].remote_prefix);
					if(config->bridges[i].topics[j].local_topic) _eecloud_free(config->bridges[i].topics[j].local_topic);
					if(config->bridges[i].topics[j].remote_topic) _eecloud_free(config->bridges[i].topics[j].remote_topic);
				}
				_eecloud_free(config->bridges[i].topics);
			}
			if(config->bridges[i].notification_topic) _eecloud_free(config->bridges[i].notification_topic);
#ifdef WITH_TLS
			if(config->bridges[i].tls_version) _eecloud_free(config->bridges[i].tls_version);
			if(config->bridges[i].tls_cafile) _eecloud_free(config->bridges[i].tls_cafile);
#ifdef REAL_WITH_TLS_PSK
			if(config->bridges[i].tls_psk_identity) _eecloud_free(config->bridges[i].tls_psk_identity);
			if(config->bridges[i].tls_psk) _eecloud_free(config->bridges[i].tls_psk);
#endif
#endif
		}
		_eecloud_free(config->bridges);
	}
#endif
	if(config->auth_plugin) _eecloud_free(config->auth_plugin);
	if(config->auth_options){
		for(i=0; i<config->auth_option_count; i++){
			_eecloud_free(config->auth_options[i].key);
			_eecloud_free(config->auth_options[i].value);
...
{
	printf("eecloud version %s (build date %s)\n\n", VERSION, TIMESTAMP);
	printf("eecloud is an MQTT v3.1 broker.\n\n");
	printf("Usage: eecloud [-c config_file] [-d] [-h] [-p port]\n\n");
...
int mqtt3_config_parse_args(struct mqtt3_config *config, int argc, char *argv[])
{
	...
	/* Default to drop to eecloud user if we are privileged and no user specified. */
	if(!config->user){
		config->user = "eecloud";
	}
	if(config->verbose){
...
#endif
	/* Default to drop to eecloud user if no other user specified. This must
				...
				if(!strcmp(token, "acl_file")){
					...
					if(reload){
						if(config->clientid_prefixes){
							_eecloud_free(config->clientid_prefixes);
							config->clientid_prefixes = NULL;
						}
					}
					...
					if(token){
						...
						token = strtok_r(NULL, " ", &saveptr);
						if(token){
							cur_listener->host = _eecloud_strdup(token);
						}else{
					...
					if(reload){
						if(config->password_file){
							_eecloud_free(config->password_file);
							config->password_file = NULL;
						}
					}
