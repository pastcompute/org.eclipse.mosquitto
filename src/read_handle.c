...
int mqtt3_packet_handle(struct eecloud_db *db, struct eecloud *context)
{
	...
	switch((context->in_packet.command)&0xF0){
		case PINGREQ:
			return _eecloud_handle_pingreq(context);
		case PINGRESP:
			return _eecloud_handle_pingresp(context);
		case PUBACK:
			return _eecloud_handle_pubackcomp(db, context, "PUBACK");
		case PUBCOMP:
			return _eecloud_handle_pubackcomp(db, context, "PUBCOMP");
		case PUBLISH:
			return mqtt3_handle_publish(db, context);
		case PUBREC:
			return _eecloud_handle_pubrec(context);
		case PUBREL:
			return _eecloud_handle_pubrel(db, context);
		case CONNECT:
		...
		case SUBACK:
			return _eecloud_handle_suback(context);
		case UNSUBACK:
			return _eecloud_handle_unsuback(context);
#endif
		default:
...
int mqtt3_handle_publish(struct eecloud_db *db, struct eecloud *context)
{
	...
	if(_eecloud_read_string(&context->in_packet, &topic)) return 1;
	if(strlen(topic) == 0){
	...
	if(context->bridge && context->bridge->topics && context->bridge->topic_remapping){
		for(i=0; i<context->bridge->topic_count; i++){
			...
			if((cur_topic->direction == bd_both || cur_topic->direction == bd_in) 
				...
				rc = eecloud_topic_matches_sub(cur_topic->remote_topic, topic, &match);
				if(rc){
					_eecloud_free(topic);
					return rc;
				}
	...
	if(qos > 0){
		if(_eecloud_read_uint16(&context->in_packet, &mid)){
			_eecloud_free(topic);
			return 1;
		}
	}
	...
	if(payloadlen){
		...
		if(_eecloud_read_bytes(&context->in_packet, payload, payloadlen)){
			_eecloud_free(topic);
	...
	switch(qos){
		...
		case 1:
			if(mqtt3_db_messages_queue(db, context->id, topic, qos, retain, &stored)) rc = 1;
			if(_eecloud_send_puback(context, mid)) rc = 1;
			break;
		case 2:
			if(!dup){
				res = mqtt3_db_message_insert(db, context, mid, ecld_md_in, qos, retain, stored);
			}else{
				res = 0;
			}
			...
			if(!res){
				if(_eecloud_send_pubrec(context, mid)) rc = 1;
			}else if(res == 1){
				rc = 1;
			}
			break;
	...
	switch(qos){
		...
		case 2:
			...
			if(!stored){
				...
				res = mqtt3_db_message_insert(db, context, mid, ecld_md_in, qos, false, stored);
			}else{
				res = 0;
			}
			if(!res){
				res = _eecloud_send_pubrec(context, mid);
			}
			return res;
	}
	return 1;
