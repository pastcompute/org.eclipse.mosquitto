...
extern int tls_ex_index_ecld;

int _eecloud_server_certificate_verify(int preverify_ok, X509_STORE_CTX *ctx)
{
	...
	if(ecld->tls_insecure == false){
		if(X509_STORE_CTX_get_error_depth(ctx) == 0){
			...
			return _eecloud_verify_certificate_hostname(cert, ecld->bridge->addresses[ecld->bridge->cur_address].address);
#else
			return _eecloud_verify_certificate_hostname(cert, ecld->host);
#endif
		}else{
			return preverify_ok;
		}
...
int eecloud__cmp_hostname_wildcard(char *certname, const char *hostname)
{
	int i;
	int len;
 ...
 */
int _eecloud_verify_certificate_hostname(X509 *cert, const char *hostname)
{
	int i;
	char name[256];
	X509_NAME *subj;
	...
	if(san){
		for(i=0; i<sk_GENERAL_NAME_num(san); i++){
			...
			if(nval->type == GEN_DNS){
				...
				if(data && !eecloud__cmp_hostname_wildcard((char *)data, hostname)){
					return 1;
				}
				have_san_dns = true;
	...
	if(X509_NAME_get_text_by_NID(subj, NID_commonName, name, sizeof(name)) > 0){
		...
		if (!eecloud__cmp_hostname_wildcard(name, hostname)) return 1;
	}
	return 0;
}
