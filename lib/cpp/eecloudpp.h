...
#		define ecldpp_EXPORT  __declspec(dllexport)
#	else
#		define ecldpp_EXPORT  __declspec(dllimport)
#	endif
#else
#	define ecldpp_EXPORT
#endif
...
ecldpp_EXPORT const char *strerror(int ecld_errno);
ecldpp_EXPORT const char *connack_string(int connack_code);
ecldpp_EXPORT int sub_topic_tokenise(const char *subtopic, char ***topics, int *count);
ecldpp_EXPORT int sub_topic_tokens_free(char ***topics, int count);
ecldpp_EXPORT int lib_version(int *major, int *minor, int *revision);
ecldpp_EXPORT int lib_init();
ecldpp_EXPORT int lib_cleanup();
ecldpp_EXPORT int topic_matches_sub(const char *sub, const char *topic, bool *result);

/*
 * Class: eecloudpp
 *
 * A eecloud client class. This is a C++ wrapper class for the eecloud C
 * library. Please see eecloud.h for details of the functions.
 */
class ecldpp_EXPORT eecloudpp {
	private:
		struct eecloud *m_ecld;
	public:
		eecloudpp(const char *id=NULL, bool clean_session=true);
		virtual ~eecloudpp();
		...
		int opts_set(enum ecld_opt_t option, void *value);
		...
		virtual void on_message(const struct eecloud_message *message) {return;};
